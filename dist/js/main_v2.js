...
import { FBXLoader } from "three/addons/loaders/FBXLoader.js"; // Add FBXLoader for armature and animations
...
function ensureStylesheetsLoaded() {
  ...
  if (!document.querySelector("style#disabled-labels-style")) {
    ...
    disabledStyle.textContent = `
      ...
      /* Style for selected body part - bigger size */
      .selected-part {
...
function createShaderBackground() {
  ...
  return {
    ...
    // Add a resize handler to update the resolution uniform
...
camera.position.set(0, 1.2, 1.6); // Position camera in front of model
...
// Add variables for body part highlighting
let hoveredBodyPartName = null;
let isCameraFocused = false;
const originalMaterialColors = {};
...
// Add global variables for armature visualization
let armatureVisualization = null;
let isArmatureVisible = false;
...
function onMouseMove(event) {
  ...
  if (isRigged) {
    ...
    resetAllBodyPartHighlights(); // Reset any highlights when rigged
    return;
  }
  ...
  // Always update highlighting regardless of camera focus state
  if (hoveredPartName !== hoveredBodyPartName) {
    hoveredBodyPartName = hoveredPartName;
    updateBodyPartHighlighting(hoveredPartName);
  }
}

// Function to update body part highlighting
function updateBodyPartHighlighting(hoveredPartName) {
  // Go through all body parts and adjust their materials
  Object.keys(bodyParts).forEach((partName) => {
    const meshData = bodyPartMeshes[partName];
    if (meshData && meshData.meshes) {
      meshData.meshes.forEach((mesh) => {
        if (!originalMaterialColors[partName]) {
          // Store original colors if not already stored
          originalMaterialColors[partName] = mesh.material.color.clone();
        }

        // Get the currently selected part (if camera is focused)
        const selectedPart = isCameraFocused
          ? document.querySelector(".selected-part")?.id.replace("label-", "")
          : null;

        if (hoveredPartName === null && !selectedPart) {
          // Reset to original color if nothing is hovered or selected
          mesh.material.color.copy(originalMaterialColors[partName]);
        } else if (partName === hoveredPartName) {
          // Brighten hovered part
          const brightenedColor = originalMaterialColors[partName]
            .clone()
            .multiplyScalar(1.2);
          mesh.material.color.copy(brightenedColor);
        } else if (partName === selectedPart && hoveredPartName === null) {
          // Keep selected part bright if it's the camera focus and nothing is hovered
          const brightenedColor = originalMaterialColors[partName]
            .clone()
            .multiplyScalar(1.2);
          mesh.material.color.copy(brightenedColor);
        } else {
          // Darken non-hovered, non-selected parts
          const darkenedColor = originalMaterialColors[partName]
            .clone()
            .multiplyScalar(0.4);
...
// Function to reset all body part highlights to original state
function resetAllBodyPartHighlights() {
  Object.keys(bodyParts).forEach((partName) => {
    const meshData = bodyPartMeshes[partName];
    if (meshData && meshData.meshes && originalMaterialColors[partName]) {
      meshData.meshes.forEach((mesh) => {
        mesh.material.color.copy(originalMaterialColors[partName]);
      });
    }
  });
  hoveredBodyPartName = null;
}
...
// Store original camera position, target and FOV
const originalCameraPosition = new THREE.Vector3(0, 1.2, 1.6);
const originalCameraTarget = new THREE.Vector3(0, 1, 0);
const originalCameraFOV = 75; // Store the original FOV value
let cameraAnimationId = null;

// Function to animate camera movement
function animateCamera(targetPosition, targetLookAt, targetFOV) {
  ...
  const startFOV = camera.fov; // Add starting FOV
...
function focusOnBodyPart(partName) {
  ...
  let cameraOffset;
  // Add a lookAt offset to adjust where the camera is pointing
  let targetOffset = new THREE.Vector3(0, 0, 0);

  // Adjust camera position based on which body part we're focusing on
  switch (partName) {
    case "head":
      cameraOffset = new THREE.Vector3(0, -0.3, 1.2);
      targetOffset = new THREE.Vector3(0, -0.2, 0); // Look slightly down at the face
      break;
    case "torso":
      cameraOffset = new THREE.Vector3(0, -0.2, 1.4);
      targetOffset = new THREE.Vector3(0, 0, 0); // Center on torso
      break;
    case "leftArm":
      cameraOffset = new THREE.Vector3(-0.5, 0.2, 1.0);
      targetOffset = new THREE.Vector3(0.1, 0, 0); // Look slightly inward
      break;
    case "rightArm":
      cameraOffset = new THREE.Vector3(0.5, 0.2, 1.0);
      targetOffset = new THREE.Vector3(-0.1, 0, 0); // Look slightly inward
      break;
    case "leftLeg":
      cameraOffset = new THREE.Vector3(-0.2, -0.6, 1.0);
      targetOffset = new THREE.Vector3(0.1, 0.2, 0); // Look slightly up and inward
      break;
    case "rightLeg":
      cameraOffset = new THREE.Vector3(0.2, -0.6, 1.0);
      targetOffset = new THREE.Vector3(-0.1, 0.2, 0); // Look slightly up and inward
  ...
  // Apply the target offset to lookAtPosition
  lookAtPosition.add(targetOffset);

  // Use a narrower FOV when focusing on a body part (zoom effect)
  const focusFOV = 35; // Lower FOV creates a zoom effect

  // Don't reset highlights when focusing on a part, just update for the focused part
  // This allows hovering to still work while a part is focused
  updateBodyPartHighlighting(hoveredBodyPartName);

  // Animate camera to new position with narrower FOV
  animateCamera(targetPosition, lookAtPosition, focusFOV);
}

// Function to reset camera to original position
function resetCameraView() {
  // Reset camera focus state
  isCameraFocused = false;

  // Animate camera back to original position with original FOV
  animateCamera(
  ...
  // Reset all body part highlights
  resetAllBodyPartHighlights();
...
function createResetViewPanel() {
  ...
  function createAndShowPanel() {
    ...
    resetViewText.textContent = "Retour";
...
const textureOptions = [
  { name: "Tissu", url: "textures/fabric.jpg" },
  { name: "Métale", url: "textures/metal.jpg" },
  { name: "Bois", url: "textures/wood.jpg" },
  { name: "Rocher", url: "textures/stone.jpg" },
  { name: "Cuir", url: "textures/leather.jpg" },

  // 15 new textures with customizable names
  { name: "Rocher 2", url: "textures/texture6.jpg" },
  { name: "Bambou", url: "textures/texture7.jpg" },
  { name: "Noir", url: "textures/texture8.jpg" },
  { name: "Marbre", url: "textures/texture9.jpg" },
  { name: "Toit", url: "textures/texture10.jpg" },
  { name: "Rouille", url: "textures/texture11.jpg" },
  { name: "Platre", url: "textures/texture12.jpg" },
  { name: "Bois 2", url: "textures/texture13.jpg" },
  { name: "Jaune", url: "textures/texture14.jpg" },
  { name: "Mur", url: "textures/texture15.jpg" },
  { name: "Rose", url: "textures/texture16.jpg" },
  { name: "Orange", url: "textures/texture17.jpg" },
  { name: "Bois 3", url: "textures/texture18.jpg" },
  { name: "Rouge", url: "textures/texture19.jpg" },
  { name: "Terre", url: "textures/texture20.jpg" },
];
...
function loadBodyPartModel(partName, typeNumber) {
  ...
  gltfLoader.load(
    ...
    function (gltf) {
      ...
      const textureName = bodyPartTextures[partName] || "Tissu";
...
function showCustomizationPanel(partName) {
  ...
  function createMainPanel() {
    ...
    const currentTextureName = bodyPartTextures[partName] || "Tissu";
...
function createRandomOptionsPanel(partName) {
  ...
  randomTypeButton.textContent = "Type aléatoire";
  ...
  randomTextureButton.textContent = "Texture aléatoire";
  ...
  randomBothButton.textContent = "Randomiser les deux";
...
function showNotification(message, type = "info") {
  // Log to console instead of showing on screen
  console.log(`[${type}] ${message}`);

  // No longer create or append any DOM elements
}
...
window.addEventListener("resize", () => {
  ...
  // Update the shader background resolution
  shaderBackground.updateResolution(window.innerWidth, window.innerHeight);
...
function updateLabelPositions() {
  ...
  Object.keys(bodyParts).forEach((partName) => {
    ...
    if (vector.z > -1 && vector.z < 1) {
      ...
      // We need to include the translate in our transform, but keep any scaling from selected state
      // Use transform for positioning but maintain scale if item is selected
      const isSelected = label.classList.contains("selected-part");
      const scale = isSelected ? 1.25 : 1 + (1 - Math.abs(vector.z)) * 0.2; // Keep larger scale if selected

      // Apply transform with translate and conditional scale
      label.style.transform = `translate(-50%, -50%) ${
        !isSelected ? `scale(${scale})` : ""
      }`;
      label.style.left = `${x}px`;
      label.style.top = `${y}px`;

      // Optional: Scale fontSize based on distance only if not selected
      if (!isSelected) {
        label.style.fontSize = `${Math.max(12, 14 * scale)}px`;
      } else {
        label.style.fontSize = `${Math.max(12, 14 * 1.25)}px`; // Fixed larger size for selected
      }

      // Optional: Adjust opacity based on distance, but only if not selected
      if (!isSelected) {
        const opacity = Math.min(
          1,
          Math.max(0.3, 1 - Math.abs(vector.z) * 0.5)
        );
...
function unrigCharacter() {
  ...
  try {
    ...
    const visualizeButton = document.getElementById("visualizeArmatureButton");
    if (visualizeButton) {
      document.body.removeChild(visualizeButton);
    }
...
function applyArmatureToCharacter(unifiedCharacter, referenceData = null) {
  ...
  const boneToPartMap = {
    ...
    // Left arm bones - only attach to the shoulder
    LeftShoulder: {
      part: "leftArm",
    ...
    // Right arm bones - only attach to the shoulder
    RightShoulder: {
      part: "rightArm",
      offset: new THREE.Vector3(0, 1.5, 0.5),
      meshRotation: new THREE.Euler(0, 0, 0),
    ...
    // Left leg bones - only attach to up leg
    LeftUpLeg: {
      part: "leftLeg",
      section: "upper",
      offset: new THREE.Vector3(0, 0.8, 0), // Increased Y offset for more noticeable positioning
      meshRotation: new THREE.Euler(0, 0, 0),
    },

    // Right leg bones - only attach to up leg
    RightUpLeg: {
      part: "rightLeg",
      section: "upper",
      offset: new THREE.Vector3(0, 0.8, 0), // Increased Y offset for more noticeable positioning
  ...
  // Find all bones in the armature to better understand the skeleton structure
  const allBones = [];
  riggedArmature.traverse((node) => {
    if (node.isBone) {
      allBones.push({
        name: node.name,
        bone: node,
  ...
  // Analyze the bones to determine best attachment points
  // This helps with finding feet bones vs thigh bones for legs
  const analyzedBones = analyzeBoneStructure(allBones);

  // Add all body parts to appropriate bones on the armature
  Object.entries(meshGroups).forEach(([bodyPart, meshes]) => {
    // For legs, we need to split meshes between upper and lower parts
    if (bodyPart === "leftLeg" || bodyPart === "rightLeg") {
      // Find upper bone
      const upperBone = findBoneForBodyPart(
        bodyPart,
        "upper",
        riggedArmature,
        mixamoBoneMap
      ...
      console.log(
        `For ${bodyPart}, found bones:`,
        upperBone ? upperBone.name : "none"
      );

      if (upperBone) {
        // Create container for the leg
        const legContainer = new THREE.Group();
        legContainer.name = `${bodyPart}Container`;

        // Get the offset for this leg from the bone map
        const offsetData =
          bodyPart === "leftLeg"
            ? mixamoBoneMap["mixamorigLeftUpLeg"] || boneToPartMap["LeftUpLeg"]
            : mixamoBoneMap["mixamorigRightUpLeg"] ||
              boneToPartMap["RightUpLeg"];

        // Apply offset to the container if available
        if (offsetData && offsetData.offset) {
          legContainer.position.copy(offsetData.offset);
        }

        upperBone.add(legContainer);

        // Add meshes to the container with proper rotation
        const meshGroup = new THREE.Group();
        meshGroup.name = `${bodyPart}MeshGroup`;

        // Apply rotation based on which leg it is
        if (bodyPart === "leftLeg") {
          meshGroup.rotation.set(Math.PI, Math.PI, 0);
        } else {
          meshGroup.rotation.set(Math.PI, Math.PI, 0);
        }

        // Add all meshes to the mesh group
        meshes.forEach((mesh) => {
        ...
        legContainer.add(meshGroup);

        console.log(
          `Applied offset (${offsetData?.offset.x}, ${offsetData?.offset.y}, ${offsetData?.offset.z}) to ${bodyPart}`
        );
      } else {
        console.warn(`Could not find appropriate bone for ${bodyPart}`);

        // Fallback: Add to a generic container
    ...
    // For arms, head, and torso
      ...
      riggedArmature.traverse((bone) => {
        ...
        if (
          boneToPartMap[bone.name] &&
          boneToPartMap[bone.name].part === bodyPart
        ) {
          targetBone = bone;
          offsetPosition =
            boneToPartMap[bone.name].offset || new THREE.Vector3(0, 0, 0);
          meshRotation = boneToPartMap[bone.name].meshRotation;
        }
        // Check mixamo mapping
        else if (
          mixamoBoneMap[bone.name] &&
          mixamoBoneMap[bone.name].part === bodyPart
        ) {
          targetBone = bone;
          offsetPosition =
            mixamoBoneMap[bone.name].offset || new THREE.Vector3(0, 0, 0);
          meshRotation = mixamoBoneMap[bone.name].meshRotation;
        }
        // Check name includes
        else if (
          bodyPart === "head" &&
          (bone.name.includes("Head") || bone.name.includes("Neck"))
        ) {
          targetBone = bone;
        } else if (
          bodyPart === "torso" &&
          ...
          bodyPart === "leftArm" &&
          (bone.name.includes("LeftArm") || bone.name.includes("LeftShoulder"))
        ) {
          targetBone = bone;
          ...
          (bone.name.includes("RightArm") ||
            bone.name.includes("RightShoulder"))
        ) {
          targetBone = bone;
          meshRotation = new THREE.Euler(Math.PI, 0, 0);
        }
      ...
      if (targetBone) {
        ...
        console.warn(
          `Could not find appropriate bone for ${bodyPart}, using fallback`
        );
...
// Function to analyze bone structure to figure out hierarchy like foot bones vs thigh bones
function analyzeBoneStructure(bones) {
  const result = {
    legs: {
      left: { upper: null, lower: null, foot: null },
      right: { upper: null, lower: null, foot: null },
    },
  };

  // Find leg bones
  bones.forEach((boneData) => {
    const name = boneData.name.toLowerCase();

    // Left leg
    if (
      name.includes("leftupleg") ||
      name.includes("left_upleg") ||
      name.includes("left_thigh")
    ) {
      result.legs.left.upper = boneData;
    } else if (name.includes("leftleg") && !name.includes("leftupleg")) {
      result.legs.left.lower = boneData;
    } else if (
      name.includes("leftfoot") ||
      name.includes("left_foot") ||
      name.includes("left_ankle")
    ) {
      result.legs.left.foot = boneData;
    }

    // Right leg
    else if (
      name.includes("rightupleg") ||
      name.includes("right_upleg") ||
      name.includes("right_thigh")
    ) {
      result.legs.right.upper = boneData;
    } else if (name.includes("rightleg") && !name.includes("rightupleg")) {
      result.legs.right.lower = boneData;
    } else if (
      name.includes("rightfoot") ||
      name.includes("right_foot") ||
      name.includes("right_ankle")
    ) {
      result.legs.right.foot = boneData;
    }
  });
...
// Function to find the appropriate bone for a body part and section
function findBoneForBodyPart(bodyPart, section, armature, boneMap) {
  let targetBone = null;
  ...
  armature.traverse((bone) => {
    ...
    // If bone is already found, skip
    if (targetBone) return;

    const name = bone.name.toLowerCase();

    // Check the bone map first
    for (const [boneName, data] of Object.entries(boneMap)) {
      if (
        bone.name === boneName &&
        data.part === bodyPart &&
...
// Normalize skeleton weights to prevent warnings
      ...
      (fbx) => {
        ...
        if (fbx.animations && fbx.animations.length > 0) {
          ...
          animations.push({
            ...
            index: index, // Store the original index to maintain order
          });
        ...
        if (loadedCount === totalAnimations) {
          // Sort animations by their original index to maintain consistent order
          animations.sort((a, b) => a.index - b.index);
